...
async function getBestYield({ token }: { token: string }) {
  ...
  return `Best yield: ${protocol} with ${
    apyBasisPoints / 100
  }% APY (Risk: ${riskLevel})`;
}

async function analyzeTrades({ user }: { user: string }) {
  return tradeAnalyzer.read.suggestStrategy([user]);
}

async function initiateFiatSwap({
  user,
  fiatAmount,
  fiatCurrency,
}: {
  ...
  return `Initiated swap: ${fiatAmount} ${fiatCurrency} to KAIA. Payment status: ${
    paymentResponse.success ? "Completed" : "Failed"
  }`;
}

async function initializeDemoAccount({ user }: { user: string }) {
  await demoTrading.read.initializeDemoAccount([user]);
  return `Demo account initialized for ${user}`;
}

async function suggestTrade({ user, token }: { user: string; token: string }) {
...
async function handleFiatSwapChat({
  message,
  userAddress,
}: {
  message: string;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case "getBestYield":
      return { result: await getBestYield(args as { token: string }) };
    case "analyzeTrades":
      return { result: await analyzeTrades(args as { user: string }) };
    case "initiateFiatSwap":
      return {
        result: await initiateFiatSwap(
          args as { user: string; fiatAmount: string; fiatCurrency: string }
        ),
      };
    case "initializeDemoAccount":
      return { result: await initializeDemoAccount(args as { user: string }) };
    case "suggestTrade":
      return {
        result: await suggestTrade(args as { user: string; token: string }),
      };
    case "handleFiatSwapChat":
      return {
        result: await handleFiatSwapChat(
          args as { message: string; userAddress: string }
        ),
      };
    default:
